...
fn test_gradient_descent_simple() {
    ...
    let result = gradient_descent(simple, 0.01);
    assert!(within_a_tenth(result[0], 1.0));
}

#[test]
fn test_gradient_descent_complex() {
    ...
    let result = gradient_descent(complex, 0.01);
...
fn test_gradient_vectorized() {
    ...
    let result = gradient_descent(vectorize!(quadratic_regression, 3), 0.01);
